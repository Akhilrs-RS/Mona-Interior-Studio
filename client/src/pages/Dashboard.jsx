...
const Dashboard = () => {
  ...
  const StatCard = ({ title, value, sub, icon: Icon, iconBg, iconText, trend, isUp }) => (
    <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between h-36 hover:shadow-md transition-shadow">
  ...
  const SectionTitle = ({ children }) => (
    <div className="flex items-center gap-3 mb-5">
      <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] whitespace-nowrap">{children}</h2>
  ...
  return (
    <div className="p-6 md:p-8 bg-slate-50 min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Business Dashboard</h1>
          <p className="text-slate-500 font-medium text-sm mt-0.5">Mona Interior — Real‑time intelligence, filtered by date.</p>
        </div>

        {/* Date Range Picker */}
        <div className="flex items-center gap-3 bg-white border border-slate-100 rounded-2xl shadow-sm p-3">
          <div className="flex flex-col">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">From</span>
            <input type="date" value={dateFrom} max={dateTo} onChange={e => setDateFrom(e.target.value)}
              className="text-sm font-black text-slate-900 bg-transparent outline-none cursor-pointer w-36" />
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard title="Billed (Revenue)"   value={fmt(totalRevenueBilled)} icon={IndianRupee} iconBg="bg-indigo-50"  iconText="text-indigo-600" sub="Total Invoiced" />
        <StatCard title="Collected (Income)" value={fmt(totalCollected)}     icon={TrendingUp}  iconBg="bg-emerald-50" iconText="text-emerald-600" sub={`${paidCount} Invoices Paid`} trend={paidCount > 0 ? "Received" : undefined} isUp />
        <StatCard title="Pending Payments"   value={fmt(totalPending)}       icon={Activity}    iconBg="bg-amber-50"   iconText="text-amber-600" sub={`${pendingCount} Invoices Due`} trend={pendingCount > 0 ? `${pendingCount} Unpaid` : undefined} isUp={false} />
        <StatCard title="Net Profit"         value={fmt(netProfit)}          icon={Receipt}     iconBg={netProfit >= 0 ? "bg-emerald-50" : "bg-rose-50"} iconText={netProfit >= 0 ? "text-emerald-600" : "text-rose-600"} trend={netProfit >= 0 ? "Profitable" : "Loss"} isUp={netProfit >= 0} />
      </div>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={getRevenueTrend()} margin={{ top:5, right:10, left:0, bottom:5 }} barCategoryGap="30%">
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize:11, fontWeight:700, fill:'#94a3b8' }} tickFormatter={v => `₹${v/1000}k`} width={45} />
                <Tooltip
        ...
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            <div className="bg-blue-50 rounded-2xl p-4 text-center">
              <p className="text-2xl font-black text-blue-700">{totalLeads}</p>
            ...
            <div className="bg-amber-50 rounded-2xl p-4 text-center">
              <p className="text-2xl font-black text-amber-700">{convRate}%</p>
        ...
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="bg-indigo-50 rounded-2xl p-4 text-center">
              <p className="text-2xl font-black text-indigo-700">{activeSites}</p>
              <p className="text-[10px] font-black text-indigo-500 uppercase tracking-wider mt-0.5">Active Sites</p>
            </div>
            <div className="bg-emerald-50 rounded-2xl p-4 text-center">
              <p className="text-2xl font-black text-emerald-700">{completedSites}</p>
            ...
            <div className="h-48">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={siteStatus} barSize={32} margin={{ top:0, right:10, left:-10, bottom:0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize:11, fontWeight:700, fill:'#94a3b8' }} />
                  <YAxis axisLine={false} tickLine={false} allowDecimals={false} tick={{ fontSize:11, fontWeight:700, fill:'#94a3b8' }} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <StatCard title="Total Staff"    value={employees.length} icon={Users}     iconBg="bg-blue-50"   iconText="text-blue-600"   sub={`${activeEmployees} Active`} />
        <StatCard title="Present Today"  value={presentToday}     icon={Calendar}  iconBg="bg-emerald-50" iconText="text-emerald-600" sub={`${absentToday} Absent`} />
        <StatCard title="Monthly Payroll" value={fmt(monthlyPayroll)} icon={Wallet} iconBg="bg-purple-50" iconText="text-purple-600" />
        <StatCard title="Active Projects" value={activeSites}     icon={Building}  iconBg="bg-amber-50"  iconText="text-amber-600" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Recent Invoices */}
        <div className="lg:col-span-2 bg-white rounded-[32px] border border-slate-100 shadow-sm p-6">
          <h3 className="font-black text-slate-900 mb-4 flex items-center gap-2"><FileText className="text-indigo-500" size={18}/> Recent Invoices</h3>
            ...
            <tbody className="divide-y divide-slate-50">
              {invoices.slice(-5).reverse().map((inv, i) => (
                <tr key={i} className="hover:bg-slate-50/50 transition">
                  <td className="py-3 px-2 font-bold text-sm text-slate-900">{inv.clientName || "—"}</td>
                  <td className="py-3 px-2 text-xs font-bold text-slate-400">{inv.date}</td>
                  <td className="py-3 px-2 text-right font-black text-slate-900 text-sm">{fmt(inv.total)}</td>
                  <td className="py-3 px-2 text-right">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider ${inv.status === "Paid" ? "bg-emerald-50 text-emerald-600" : inv.status === "Draft" ? "bg-slate-100 text-slate-500" : "bg-amber-50 text-amber-600"}`}>
                      {inv.status}
                    </span>
                  </td>
                </tr>
        ...
        {/* Quick Actions */}
        <div className="bg-slate-900 rounded-[32px] p-6 text-white shadow-xl flex flex-col justify-between">
          <div>
