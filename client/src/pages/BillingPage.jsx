...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    if (location.state?.convertQuote) {
      ...
      setItems(mapped);
      setEditBadge(`Converted from Quotation (${isNonGST ? 'Non-GST' : 'GST'}) — Review & save as Invoice`);
      return;
    }
  ...
  const addItem = (e) => {
    ...
    const effectiveGST = billType === "Non-GST" ? 0 : parseFloat(newItem.gstPerc || 0);
    ...
    setNewItem({ work: "", unit: "Sq.Ft", area: "", price: "", gstPerc: billType === "Non-GST" ? 0 : 18 });
  };
    ...
    <div className="bg-gray-200 min-h-screen font-sans text-slate-800 flex flex-col">
      ...
      {editBadge && (
        <div className={`px-4 py-2 flex items-center justify-between text-xs font-bold ${
          isEditMode ? "bg-violet-600 text-white" : "bg-emerald-600 text-white"
        }`}>
          ...
          <button
            onClick={() => { setEditBadge(""); setIsEditMode(false); }}
      ...
      <div className="bg-blue-50 p-2 grid grid-cols-12 gap-2 border-b border-blue-200 items-end text-slate-600">
        <div className="col-span-2">
          ...
          <div className="flex">
            ...
            <button onClick={() => navigate("/invoices")} className="bg-blue-600 text-white px-2 hover:bg-blue-700 transition" title="View Invoice History"><History size={14}/></button>
          </div>
        </div>
        <div className="col-span-2">
        ...
        <div className="col-span-1 flex flex-col items-end justify-end text-[10px] font-bold pb-0.5">
          {billType === "GST" ? (
            <>
              <div>CGST: <span className="text-blue-700">₹{(totalGst/2).toFixed(0)}</span></div>
              <div>SGST: <span className="text-blue-700">₹{(totalGst/2).toFixed(0)}</span></div>
            </>
          ) : (
            <div className="text-rose-600 font-black uppercase tracking-wider">Non-GST Bill</div>
          )}
        </div>
      </div>
        ...
        <table className="w-full text-[11px]">
          <thead className="bg-gray-100 border-b border-gray-300 sticky top-0">
            <tr className="uppercase text-gray-600 font-bold">
              ...
              <th className="px-2 py-1 border-r border-gray-300 text-right w-24">Amount</th>
              {billType === "GST" && <>
      ...
      <div className="bg-gray-100 p-2 border-t border-gray-300 flex justify-between items-end gap-4">
        ...
        <div className="flex gap-4 mb-2">
          <div className="bg-blue-50 border border-blue-200 px-3 py-1 flex gap-2 items-center">
            <span className="text-[10px] font-bold text-blue-800 uppercase">Tot Amount:</span>
          ...
          {billType === "Non-GST" && (
            <div className="bg-rose-50 border border-rose-200 px-3 py-1 flex gap-2 items-center">
              <span className="text-[10px] font-black text-rose-700 uppercase">Non-GST Bill — No Tax</span>
            </div>
          )}
        </div>
